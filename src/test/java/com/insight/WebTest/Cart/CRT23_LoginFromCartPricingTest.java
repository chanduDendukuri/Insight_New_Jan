		...
		try {
			...
			for (int intCounter = intStartRow; intCounter <= intEndRow; intCounter++) {
				try {
					...
					Hashtable<String, String> data = TestUtil.getDataByRowNo("CRT23_LoginFromCartPricing",
					...
					if (listPrice.equalsIgnoreCase(currencyCode + " " + summaryAmountNonLoggedIn)) {
						reporter.SuccessReport("Verify the Non LoggedIn list price in cart",
								"list price same as in cart", currencyCode + " " + summaryAmountNonLoggedIn);
					} else {
						reporter.failureReport("Verify the Non LoggedIn list price in cart",
								"list price is not same as in cart", summaryAmountNonLoggedIn);
					}
					...
					if (!(currencyCode + " " + priceLogin.trim())
							.equalsIgnoreCase(currencyCode + " " + summaryAmountNonLoggedIn.trim())) {
						reporter.SuccessReport("Verify the Non LoggedIn and LoggedIn list price in SBP Page",
								"	LoggedIn price changed in SBP Page",
								"LoggedIn Price: " + currencyCode + " " + priceLogin + "and Non-LoggedIn price: "
										+ currencyCode + " " + summaryAmountNonLoggedIn);
					} else {
						reporter.failureReport("Verify the Non LoggedIn and LoggedIn list price in SBP Page",
								"	LoggedIn price same in SBP Page",
								"LoggedIn Price: " + currencyCode + " " + priceLogin + "and Non-LoggedIn price: "
										+ currencyCode + " " + summaryAmountNonLoggedIn);
					}
					commonLib.clickCart();
			...
			gTestStatus = false;
			ReportStatus.fnUpdateResultStatus("LoginFromCartPricing", "Tc_CRT23", ReportStatus.strMethodName, 1,
					browser);
