...
public class CMTLib extends CMTObj {
	...
	public void searchForWebGroup(String webGroup) throws Throwable {
		if(isElementVisible(CLIENT_SEARCH_LABEL, 3, "CMT home page")) {
			reporter.SuccessReport("Verify Client Admin Search Page", "Client Admin Search Page Exists with client search label", "CMT home screen");
			waitForVisibilityOfElement(WEB_GROUP, "WEB GROUP in CMT TOOL HOME PAGE");
			...
			reporter.failureReport("Verify CMT home page displayed", "CMT home page is not displayed ", "CMT home screen");
		}
	}
	...
	public void verifyUserandClick(String contactName) throws Throwable {
		...
		if(isElementVisible(USER_MANAGEMENT_LABEL, 3, "Manage Web Groups:  User Management")) {
			reporter.SuccessReport("Verify Web Group Management: Users Page", "Web Group Management: Users Page loaded", "Page: Web Group Management: Users");
			if (actualuser.contains(contactName)) {
				reporter.SuccessReport("Verifying User Contact Name :", "User Contact Name present is : ", "Contact name : "+contactName);
				click(CMTObj.getUser(contactName), contactName);
				if(isElementVisible(USER_CONTACT_PAGE, 3, "user contaact page")) {
					reporter.SuccessReport("Verify user contact page", "Uers contact page verified successfully", "Web Group Management: Users");
				} else {
					reporter.failureReport("Verify user contact page", "Uers contact page not loaded  successfully", "", driver);
				}
			} else {
				reporter.failureReport("Verifying User Contact Name :", "User Contact Name not present is : ", contactName, driver);
			}
		}else {
			reporter.failureReport("Verify Web Group Management: Users Page", "Web Group Management: Users Page not loaded", "Page: Web Group Management: Users");
		}
	...
	public void searchForaUserAndSelect(String LnameEmailUname, String ContactName) throws Throwable {
		if(isElementVisible(USER_MANAGEMENT_LABEL, 3, "User Management page")) {
			reporter.SuccessReport("Verify Web Group Management: Users Page", "User management page is displayed", "Page: Web Group Management: Users");
			Thread.sleep(5000);
	...
	public void setPermissions(String menuName, String userPermissions) throws Throwable {
		...
		if (isCheckBoxSelected(getUserPermission(userPermissions))) {
			...
			reporter.SuccessReport(userPermissions, "check box already checked::"+userPermissions+"",userPermissions);
		} else {
	...
	public void setPermissionsToDisable(String menuName, String userPermissions) throws Throwable {
		...
		if (isCheckBoxSelected(getUserPermission(userPermissions))) {
			...
			reporter.SuccessReport("Verify Customer Level Permissions", "User permissions :"+userPermissions+"check box already Disabled", ""+userPermissions+"");
		}
	}
...
//		}
	...
	public void setCustomerLevelPermissionsOFF(String customerPermissions) throws Throwable {
		...
		if (isCheckBoxSelected(getCustomerLevelPermissionsForWebGrp(customerPermissions))) {
			click(getCustomerLevelPermissionsForWebGrp(customerPermissions), "Web Group level permissions::"+customerPermissions+" is OFF");
			click(UPDATE_CUSTOMER_PERMISSIONS_BTN, "Update button");
			if (isElementPresent(CUSTOMER_PERMISSION_UPDATE_MSG, "update sucessful message")) {
				reporter.SuccessReport("Verify the Success message ", "Permissions Updated Succesfully", ""+customerPermissions+"");
			} else {
				reporter.failureReport("Verify the sucess message", "Permissions are not Updated Succesfully", "", driver);
			}
		} else {
			LOG.info(customerPermissions + " check box already Unchecked.");
			reporter.SuccessReport("Verify Web Group Level Permissions ", "Web Group level permissions::"+customerPermissions+" is OFF", ""+customerPermissions+"");
		}
	}
    ...
    */
	 ...
	 */
		...
		/**
		 * This method is to verify the error messsage when user driectly clicks on create user
		 * @throws Throwable
		 */
